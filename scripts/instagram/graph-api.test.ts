...
describe("buildMediaUrl", () => {
  test("targets the user's media edge with all required fields", () => {
    ...
    expect(url.origin + url.pathname).toBe("https://graph.instagram.com/v21.0/178414/media");
...
describe("fetchRecentMedia", () => {
  ...
  test("throws with status and body on a non-ok response", async () => {
    ...
    await expect(fetchRecentMedia(config, fakeFetch as unknown as typeof fetch)).rejects.toThrow(/401.*invalid token/);
  });
});
